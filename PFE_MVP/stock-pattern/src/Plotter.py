...
class Plotter:
    ...
    def plot(self, idx=None) -> None:
        ...
        if self.save_folder:
            json_path = self.save_folder / f"{sym}_{pattern}_{self.timeframe}.json"
            self.export_json(dct, pattern, df, lines, json_path)
    ...
    def export_json(self, dct: dict, pattern: str, df: pd.DataFrame, lines, out_path: Path):
        sym = dct["sym"].upper()
        ...
        payload = {
            ...
            "mode": self.mode,
            "df_range": {
                "start": self._dt_to_iso(df.index[0]),
                "end": self._dt_to_iso(df.index[-1]),
                "rows": int(len(df)),
            },
            "detection_window": {
                "df_start": self._dt_to_iso(dct.get("df_start")),
                "df_end": self._dt_to_iso(dct.get("df_end")),
                "end": self._dt_to_iso(dct.get("end")),
            },
            "candles": [
                {
                    "t": self._dt_to_iso(ts),
                    "open": float(row["Open"]),
                    "high": float(row["High"]),
                    "low": float(row["Low"]),
            ...
            "points": {
                k: {"t": self._dt_to_iso(v[0]), "price": float(v[1])}
                for k, v in dct.get("points", {}).items()
            },
            "extra_points": {
                k: {"t": self._dt_to_iso(v[0]), "price": float(v[1])}
                for k, v in dct.get("extra_points", {}).items()
            },
            "lines": [
                {
                    "p1": {"t": self._dt_to_iso(p1[0]), "price": float(p1[1])},
                    "p2": {"t": self._dt_to_iso(p2[0]), "price": float(p2[1])},
                }
                for (p1, p2) in lines
            ],
            "meta": {
                "saved_at": datetime.now().isoformat(),
                "plot_idx": int(self.idx),
            },
        }
